...
peer chaincode install -n exnet -v 1.1 -l node -p /opt/gopath/src/github.com/hyperledger/fabric/peer/chaincode/
...
peer chaincode instantiate -o orderer.example1-network.com:7050 -C example1channel -n exnet -l node -v 1.1 -c '{"Args":["org.example1-network.propnet:instantiate"]}' -P "OR ('abhiMSP.member','savaMSP.member')"
